#### Master Script 11a: Assess APM_DeepMN and APM_DeepOR performance ####
#
...
def main(array_task_id):
    ...
    metric_dir = os.path.join('../model_performance','APM',curr_model,'resample'+str(curr_rs_idx).zfill(4))
    os.makedirs(metric_dir,exist_ok=True)
    
    # Load compiled testing set predictions
    compiled_test_preds = pd.read_csv(os.path.join(model_dir,'APM_'+curr_model+'_compiled_test_predictions.csv'),index_col=0)
    ...
    pd.DataFrame({'MODEL':'APM_D'+curr_model[1:],'RESAMPLE_IDX':curr_rs_idx,'ORC':final_orc,'S':final_steps,'Gen_C':final_gen_c,'D_xy':final_D_xy,'Accuracy':final_accuracy},index=[0]).to_csv(os.path.join(metric_dir,'deep_overall_metrics.csv'),index=False)
    ...
    final_auc = pd.DataFrame({'MODEL':'APM_D'+curr_model[1:],
    ...
    final_rocs['MODEL']='APM_D'+curr_model[1:]
    ...
    for idx in range(len(thresh_prob_labels)):
        ...
        accuracies.append(accuracy_score(curr_thresh_label,curr_thresh_preds))
    final_thresh_accuracy = pd.DataFrame({'MODEL':'APM_D'+curr_model[1:],
                                          'RESAMPLE_IDX':curr_rs_idx,
                                          'Threshold':thresh_labels,
                                          'Accuracy':accuracies})
    ...
    final_calibs['MODEL']='APM_D'+curr_model[1:]
    ...
    final_calib_metrics['MODEL']='APM_D'+curr_model[1:]
    final_calib_metrics['RESAMPLE_IDX'] = curr_rs_idx

    #### Compile and save threshold-level metrics
    apm_deep_tlm = pd.merge(final_auc,final_thresh_accuracy,how='left',on=['MODEL','RESAMPLE_IDX','Threshold']).merge(final_calib_metrics,how='left',on=['MODEL','RESAMPLE_IDX','Threshold'])
    apm_deep_tlm = apm_deep_tlm[['MODEL','RESAMPLE_IDX','Threshold','AUC','Accuracy','Calib_Intercept','Calib_Slope']]
    apm_deep_tlm.to_csv(os.path.join(metric_dir,'deep_threshold_metrics.csv'),index=False)
    ...
    final_cm['MODEL']='APM_D'+curr_model[1:]
