#### Master Script 4: Train logistic regression concise-predictor-based models (CPM) ####
#
...
### Contents:
# I. Initialisation
# II. Define function to train logistic regression CPMs given repeated cross-validation dataframe
# III. Parallelised training of logistic regression CPMs and testing set prediction
...
### II. Define function to train logistic regression CPMs given repeated cross-validation dataframe 
def train_CPM_logreg(split_df,model_dir,progress_bar=True,progress_bar_desc = 'Training CPM_logreg'):
    ...
    for curr_split_row in iterator:
        ...
        training_set = pd.read_csv('../imputed_CPM_sets/repeat'+str(curr_repeat).zfill(2)+'/fold'+str(curr_fold)+'/training_set.csv')
        testing_set = pd.read_csv('../imputed_CPM_sets/repeat'+str(curr_repeat).zfill(2)+'/fold'+str(curr_fold)+'/testing_set.csv')
        ...
        training_categorical = pd.DataFrame(cat_encoder.fit_transform(training_set[['GCSm','marshall','unreactive_pupils']]).toarray(),
                                            columns=cat_column_names)
        training_set = pd.concat([training_set.drop(columns=['GCSm','marshall','unreactive_pupils']),training_categorical],axis=1)

        testing_categorical = pd.DataFrame(cat_encoder.transform(testing_set[['GCSm','marshall','unreactive_pupils']]).toarray(),
                                    columns=cat_column_names)
        testing_set = pd.concat([testing_set.drop(columns=['GCSm','marshall','unreactive_pupils']),testing_categorical],axis=1)
        ...
        training_set[['age','Hb','glu']] = num_scaler.fit_transform(training_set[['age','Hb','glu']])
        testing_set[['age','Hb','glu']] = num_scaler.transform(testing_set[['age','Hb','glu']])
...
with multiprocessing.Pool(NUM_CORES) as pool:
    pool.starmap(train_CPM_logreg, core_splits)

### IV. Compile testing set predictions
## Compile all test predictions from CPM_MNLR
...
## Compile all test predictions from CPM_POLR
