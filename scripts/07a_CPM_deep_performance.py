#### Master Script 7a: Assess CPM_DeepMN and CPM_DeepOR performance ####
#
...
def main(array_task_id):
    ...
    metric_dir = os.path.join('../model_performance','CPM',curr_model,'resample'+str(curr_rs_idx).zfill(4))
    os.makedirs(metric_dir,exist_ok=True)
    
    # Load compiled testing set predictions
    compiled_test_preds = pd.read_csv(os.path.join(model_dir,'CPM_'+curr_model+'_compiled_test_predictions.csv'),index_col=0)
    ...
    pd.DataFrame({'MODEL':'CPM_D'+curr_model[1:],'RESAMPLE_IDX':curr_rs_idx,'ORC':final_orc,'S':final_steps,'Gen_C':final_gen_c,'D_xy':final_D_xy,'Accuracy':final_accuracy},index=[0]).to_csv(os.path.join(metric_dir,'deep_overall_metrics.csv'),index=False)
    ...
    final_auc = pd.DataFrame({'MODEL':'CPM_D'+curr_model[1:],
    ...
    final_rocs['MODEL']='CPM_D'+curr_model[1:]
    ...
    for idx in range(len(thresh_prob_labels)):
        ...
        accuracies.append(accuracy_score(curr_thresh_label,curr_thresh_preds))
    final_thresh_accuracy = pd.DataFrame({'MODEL':'CPM_D'+curr_model[1:],
                                          'RESAMPLE_IDX':curr_rs_idx,
                                          'Threshold':thresh_labels,
                                          'Accuracy':accuracies})
    ...
    final_calibs['MODEL']='CPM_D'+curr_model[1:]
    ...
    final_cm['MODEL']='CPM_D'+curr_model[1:]
