#### Master Script 6b: Perform interrepeat hyperparameter configuration dropout on deep learning concise-predictor-based models (CPM) ####
#
...
if REPEAT == 1:
    ...
    tuning_grid = pd.read_csv(os.path.join(model_dir,'CPM_deep_tuning_grid.csv'))
    tuning_grid['TUNE_IDX'] = tuning_grid['TUNE_IDX'].astype(str).str.zfill(4)
    tuning_grid['NEURONS'] = tuning_grid['NEURONS'].apply(eval)
    
else:
    
    tuning_grid = pd.read_csv(os.path.join(model_dir,'CPM_post_repeat_'+str(REPEAT).zfill(2)+'_deep_tuning_grid.csv'))
    tuning_grid['TUNE_IDX'] = tuning_grid['TUNE_IDX'].astype(str).str.zfill(4)
    tuning_grid['NEURONS'] = tuning_grid['NEURONS'].apply(eval)
...
val_pred_file_info_df = pd.DataFrame({'file':val_pred_files,
                                      ...
                                      'VERSION':[re.search('CPM_outputs/(.*)/repeat', curr_file).group(1) for curr_file in val_pred_files],
...
dropout_dir = os.path.join('/home/sb2406/rds/hpc-work/interrepeat_dropout','CPM_deep',VERSION)
...
viable_tuning_grid.to_csv(os.path.join(model_dir,'CPM_post_repeat_'+str(REPEAT).zfill(2)+'_deep_tuning_grid.csv'),index=False)
...
softmax_val_preds.to_csv(os.path.join(model_dir,'CPM_deepMN_compiled_val_predictions.csv'),index=True)
softmax_test_preds.to_csv(os.path.join(model_dir,'CPM_deepMN_compiled_test_predictions.csv'),index=True)

sigmoid_val_preds.to_csv(os.path.join(model_dir,'CPM_deepOR_compiled_val_predictions.csv'),index=True)
sigmoid_test_preds.to_csv(os.path.join(model_dir,'CPM_deepOR_compiled_test_predictions.csv'),index=True)